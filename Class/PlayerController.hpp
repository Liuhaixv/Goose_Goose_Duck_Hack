...
public:
	...
	static void printAllPlayersInfo(std::list<PlayerController> playerControllers, Utils* utils = nullptr) {
		...
		std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) << (utils ? utils->str("Nickname", "玩家昵称") : "玩家昵称");
		std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) << (utils ? utils->str("Rolename", "角色") : "角色");
		std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) << (utils ? utils->str("Pos", "坐标") : "坐标");
		std::cout << '\n';
