        ...
        while (true) {
            ...
            //自动完成任务
            {
                ...
                if (state == SHOULD_ACTIVATE_NOW) {
                    ...
                    //申请的内存基址
                    static int64_t injectCodesBaseAddress = NULL;
                    ...
                    if (!b_allocatedMemory) {
                        ...
                        if (result == true) {
                            ...
                            memory.write_bytes(injectCodesBaseAddress, injectAsmCodes, sizeof(injectAsmCodes) / sizeof(byte));
                    ...
                    std::vector<byte> ASM_jmp_rax = { 0xFF, 0xE0 };

                    jmpBytes.insert(jmpBytes.end(), ASM_mov_rax.begin(), ASM_mov_rax.end());
                    jmpBytes.insert(jmpBytes.end(), injectCodesBaseAddress_LittleEndianBytes.begin(), injectCodesBaseAddress_LittleEndianBytes.end());
                    jmpBytes.insert(jmpBytes.end(), ASM_jmp_rax.begin(), ASM_jmp_rax.end());
