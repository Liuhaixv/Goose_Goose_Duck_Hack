...
void Hack::resetActivationStates() {
    this->b_hasDisabledFOW = false;
    this->b_hasEnabledNoclip = false;
}

//只需要激活一次
void Hack::removeFogOfWar(PlayerController* localPlayerController) {
...
bool Hack::enableNoclip(PlayerController* localPlayer, bool shouldEnable) {
...
void Hack::noclip(PlayerController* localPlayerController) {
    //开启穿墙
...
bool Hack::speedHack(LocalPlayer* localPlayer) {
    //目标速度
...
void Hack::zoomHack(LocalPlayer* localPlayer)
{
...
/// <returns></returns>
bool Hack::overrideOrthographicSize(LocalPlayer* localPlayer, float targetZoomSize)
{
    if (localPlayer) {
        //启用Override
